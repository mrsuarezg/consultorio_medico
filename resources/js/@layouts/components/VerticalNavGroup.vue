...
watch(() =>route().current(), () => {
      ...
      isGroupOpen.value = isActive && !isVerticalNavMini(windowWidth, props.isVerticalNavHovered).value
      isGroupActive.value = isActive
    }, { immediate: true })
...
watch(openGroups, val => {
  ...
  if (lastOpenedGroup === props.item.title)
    return
  const isActive = isNavGroupActive(props.item.children, route().current())
