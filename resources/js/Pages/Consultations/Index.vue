...
async function getPatients() {
    try {
        ...
        console.error(error);
        showSnackbar('Ha ocurrido un error al obtener los consultas', 'error', false);
    } finally {
...
function registerPatient() {
    showSmallDialog('Registrar consulta', ConsultationForm, {}, true);
}

function editCustomer(id) {
    showSmallDialog('Editar consulta', PatientForm, { patientId: id }, true);
}

function previewPatient(id) {
    showSmallDialog('Información del consulta', PatientPreviewForm, { patientId: id }, false);
}
...
<template>
    <main>
        <MultipleItems remove-margin-top @add-item="registerPatient">
            <template #title>
                <span class="mr-4">Consultas</span>
            ...
            <VDataTableServer :headers="headers" :items="items" :items-length="items.length" :items-per-page="itemsPerPage"
                ...
                <template #no-data>
                    <p class="pa-4">No hay consultas para mostrar</p>
                </template>
