...
// Get InitialThemeColors only specific in colors
const initialThemeColors = {
  ...
  success: vuetifyTheme.current.value.colors.success,
  info: vuetifyTheme.current.value.colors.info,
  warning: vuetifyTheme.current.value.colors.warning,
  error: vuetifyTheme.current.value.colors.error,
}
const colors = [
  'primary',
  'secondary',
    ...
    <VNavigationDrawer v-model="isNavDrawerOpen" temporary location="end" width="400" :scrim="false"
      ...
      <div class="customizer-heading d-flex align-center justify-space-between">
        <div>
          ...
          <span class="text-body-1">Personaliza y Visualiza en Tiempo Real</span>
        </div>
      ...
      <PerfectScrollbar tag="ul" :options="{ wheelPropagation: false }">
        ...
        <CustomizerSection title="THEMING" :divider="false">
          ...
          <div class="d-flex align-center">
            ...
            <div>
              <VSwitch id="pricing-plan-toggle" v-model="theme" label="Oscuro" true-value="dark" false-value="light" />
            </div>
          </div>
          ...
          <div class="d-flex gap-x-4 mt-2">
            <div v-for="(color, index) in colors" :key="index"
              ...
              :style="{ backgroundColor: initialThemeColors[color] }"
        ...
        <CustomizerSection title="LAYOUT">
          ...
          <!-- 👉 Navbar Type -->
          <h6 class="mt-3 text-base font-weight-regular">
            {{ appContentLayoutNav === AppContentLayoutNav.Vertical ? 'Navbar' : 'Header' }} Tipo
          </h6>
        ...
        <CustomizerSection title="MENU">
          ...
          <VRadioGroup v-model="appContentLayoutNav" inline>
            <VRadio v-for="[key, val] in Object.entries(AppContentLayoutNav)" :key="key" :label="key" :value="val" :disabled="key === 'Horizontal'" />
          </VRadioGroup>
          ...
          <div class="align-center justify-space-between"
            ...
            <div>
              <VSwitch id="customizer-menu-semi-dark" v-model="isVerticalNavSemiDark" class="ms-2" />
            </div>
          </div>
        </CustomizerSection>
        <!-- !SECTION -->
        ...
        <CustomizerSection title="MISC">
          ...
          <div class="mt-6">
            <VRow>
              ...
              <VCol cols="7">
                <VSelect id="route-transition" v-model="appRouteTransition"
                  :items="Object.entries(RouteTransitions).map(([key, value]) => ({ key, value }))" item-title="key"
                  item-value="value" single-line />
              </VCol>
            </VRow>
          </div>
        </CustomizerSection>
